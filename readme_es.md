<p align="center">
  <img src="./resources/foundry-banner-1.png" alt="Foundry - Cowork with Your CLI AI Agent" width="100%">
</p>

<p align="center">
  <img src="https://img.shields.io/github/v/release/Foundry/Foundry?style=flat-square&color=32CD32" alt="Version">
  &nbsp;
  <img src="https://img.shields.io/badge/license-Apache--2.0-32CD32?style=flat-square&logo=apache&logoColor=white" alt="License">
  &nbsp;
  <img src="https://img.shields.io/badge/platform-macOS%20%7C%20Windows%20%7C%20Linux-6C757D?style=flat-square&logo=linux&logoColor=white" alt="Platform">
</p>

<p align="center">
  <a href="https://trendshift.io/repositories/15423" target="_blank">
    <img src="https://trendshift.io/api/badge/repositories/15423" alt="GitHub Trending" height="80">
  </a>
</p>

---

<p align="center">
  <strong>🚀 Asistente personal 「Cowork」basado en Gemini CLI, Claude Code, Codex, Qwen Code, Goose CLI, Auggie y más</strong><br>
  <em>Fácil de usar | Interfaz gráfica visual | Soporte multimodelo | Seguridad de datos local</em>
</p>

<p align="center">
  <a href="https://github.com/Foundry/Foundry/releases">
    <img src="https://img.shields.io/badge/⬇️%20Descargar%20ahora-Última%20versión-32CD32?style=for-the-badge&logo=github&logoColor=white" alt="Download Latest Release" height="50">
  </a>
</p>

<p align="center">
  <a href="./readme.md">English</a> | <a href="./readme_ch.md">简体中文</a> | <a href="./readme_tw.md">繁體中文</a> | <a href="./readme_jp.md">日本語</a> | <a href="./readme_ko.md">한국어</a> | <strong>Español</strong> | <a href="./readme_pt.md">Português</a> | <a href="https://www.foundry.app" target="_blank">Sitio oficial</a> | <a href="https://twitter.com/FoundryApp" target="_blank">Twitter</a>
</p>

<p align="center">
  <strong>💬 Comunidad:</strong> <a href="https://discord.gg/2QAwJn7Egx" target="_blank">Discord (English)</a> | <a href="./resources/wx.png" target="_blank">WeChat (中文)</a>
</p>

---

## 📋 Navegación rápida

<p align="center">

[✨ ¿Qué puede hacer Foundry?](#qué-puede-hacer-foundry) ·
[🤔 ¿Por qué elegir Foundry?](#por-qué-elegir-foundry) ·
[🚀 Inicio rápido](#inicio-rápido) ·
[💬 Comunidad](#comunidad-y-soporte)

</p>

---

## ✨ ¿Qué puede hacer Foundry?

<p align="center">
  <img src="./resources/offica-ai BANNER-function.png" alt="Foundry - Cowork with Your CLI AI Agent" width="800">
</p>

### 🤖 **Modo multiagente - Cowork para tus herramientas de IA en línea de comandos, interfaz gráfica unificada**

_Foundry incluye Gemini CLI de forma integrada, listo para usar desde la instalación sin extras. Si ya tienes herramientas de línea de comandos como Gemini CLI, Claude Code, CodeX, Qwen Code, Goose AI o Augment Code, Foundry las detecta automáticamente y ofrece una interfaz gráfica unificada para una experiencia más completa_

- ✅ **Detección automática + interfaz unificada** - Reconoce automáticamente las herramientas CLI locales y proporciona una interfaz gráfica unificada, despídete de la línea de comandos
- ✅ **Almacenamiento local + multi-sesión** - Conversaciones guardadas localmente, admite múltiples sesiones en paralelo, cada una con contexto independiente

<p align="center">
  <img src="./resources/acp home page.gif" alt="Multi-Agent Mode Demo" width="800">
</p>

---

### 🌐 **Acceso a tu Foundry desde cualquier lugar**

_Tu asistente de IA 7×24 horas - Accede a Foundry desde cualquier dispositivo, en cualquier lugar. En viajes de negocios, en casa, en la oficina, usa tus herramientas de IA en cualquier momento y lugar a través de WebUI o Telegram_

Foundry proporciona dos métodos de acceso remoto：

- **🌐 Modo WebUI**

  Accede a Foundry desde cualquier dispositivo a través del navegador - teléfono, tableta, computadora. Soporta LAN, red cruzada y despliegue en servidor. Puedes iniciar sesión escaneando un código QR o usando contraseña de cuenta, haciéndolo simple y conveniente.

  > 💡 **¿Necesitas una guía de configuración detallada?** Consulta la [Guía de acceso remoto a Internet](https://github.com/Foundry/Foundry/wiki/Remote-Internet-Access-Guide)

- **📱 Telegram Channel**

  Chatea directamente con tu asistente de IA desde Telegram en cualquier dispositivo. Sistema de código de emparejamiento simple para acceso seguro.

  > 💡 **Cómo configurar:** Ve a Configuración de Foundry → Configuración de WebUI → Channel, obtén tu Bot Token de [@BotFather](https://t.me/BotFather) y comienza a chatear!

<p align="center">
  <img src="./resources/remote-telegram.png" alt="Acceso desde cualquier lugar - WebUI & Telegram" width="800">
</p>

---

### ⏰ **Tareas programadas - Deja que Foundry automatice tu trabajo**

_Después de configurar las tareas programadas, el asistente de IA se ejecutará automáticamente según el tiempo que establezcas, logrando verdaderamente una operación 7×24 horas sin supervisión_

- **Conversación libre** - Dile a la IA qué hacer usando lenguaje natural, como chatear normalmente
- **Configuración de tiempo flexible** - Diario, semanal, mensual, todo es posible
- **Simple y fácil** - Crear, modificar, habilitar/deshabilitar, eliminar, ver y ajustar en cualquier momento

<p align="center">
  <img src="./resources/alart-task.png" alt="Tareas programadas Demo" width="800">
</p>

> 💡 **Casos de uso:** Agregación de datos programada, generación de informes regulares, organización automática de archivos, recordatorios programados, etc.

---

### 📁 **Gestión inteligente de archivos (AI Cowork)**

_Renombrado por lotes, organización automática, clasificación inteligente, fusión de archivos_

- **Organización automática**: Identifica contenido de forma inteligente y clasifica automáticamente, manteniendo carpetas ordenadas.
- **Lotes eficientes**: Renombra y fusiona archivos con un clic, adiós a tareas manuales tediosas.

<p align="center">
  <img src="./resources/foundry sort file.gif" alt="Smart File Management Demo" width="800">
</p>

---

### 📄 **Panel de vista previa - Visualiza rápidamente los resultados generados por IA**

_Compatible con 9+ formatos de vista previa visual (PDF, Word, Excel, PPT, código, Markdown, imágenes, HTML, Diff, etc.)_

- ✅ **Ver resultados al instante** - Después de que la IA genere archivos, ve la vista previa inmediatamente sin cambiar de app
- ✅ **Seguimiento en tiempo real + editable** - Rastrea automáticamente cambios de archivos, editor y vista previa sincronizados; soporta edición en tiempo real de Markdown, código, HTML, WYSIWYG

<p align="center">
  <img src="./resources/preview.gif" alt="Preview Panel Demo" width="800">
</p>

---

### 🎨 **Generación y edición de imágenes con IA**

_Generación, edición y reconocimiento inteligente de imágenes, impulsado por Gemini_

<p align="center">
  <img src="./resources/Image_Generation.gif" alt="AI Image Generation Demo" width="800">
</p>

---

### 🤖 **Soporte multi-modelo**

_Admite modelos principales como Gemini, OpenAI, Claude, Qwen, y modelos locales como Ollama, LM Studio. Cambia flexiblemente entre diferentes modelos para satisfacer varios requisitos de tareas._

<p align="center">
  <img src="./resources/multi-model.png" alt="Soporte multi-modelo" width="800">
</p>

---

### 🎨 **Personalización de interfaz personalizada**

_Personaliza con tu propio código CSS, haz que tu interfaz coincida con tus preferencias_

<p align="center">
  <img src="./resources/css with skin.gif" alt="CSS Custom Interface Demo" width="800">
</p>

- ✅ **Totalmente personalizable** - Personaliza libremente colores, estilos y diseño de la interfaz a través de código CSS, crea tu experiencia exclusiva

---

### 💬 **Procesamiento paralelo de múltiples tareas**

_Abrir varias conversaciones, tareas sin mezclarse, memoria independiente, eficiencia duplicada_

<details>
<summary><strong>🎬 Haz clic para ver la demo ▶️</strong></summary>
<br>
<p align="center">
  <img src="./resources/multichat-side-by-side.gif" alt="Conversation Management Demo" width="800">
</p>
</details>

---

## 🤔 ¿Por qué elegir Foundry? | Tan bueno como Claudebot

**Al igual que Claude Cowork hace que Claude Code sea más fácil de usar, Foundry es la plataforma Cowork para todas tus herramientas de IA en línea de comandos**

Gemini CLI, Claude Code, Codex, Qwen Code son potentes, pero comparten puntos de dolor comunes: las conversaciones no se pueden guardar, limitaciones de sesión única, operaciones de archivos engorrosas y solo admiten un modelo.

Foundry ofrece capacidades **Cowork unificadas** para estas herramientas de línea de comandos:

- 🎯 **Plataforma unificada** - Una interfaz para gestionar todas las herramientas de IA de línea de comandos, sin cambios; incluye Gemini CLI, listo para usar desde la instalación y completamente gratis
- 🚀 **Soporte multi-herramienta** - No solo admite Claude Code, también Gemini CLI, Codex, Qwen Code y más
- 🖥️ **Multiplataforma** - Soporte completo para macOS, Windows y Linux (Claude Cowork solo macOS)
- 🌐 **Acceso remoto** - Tu asistente remoto 24/7, accede en cualquier momento y lugar, completamente gratis
- 🔄 **Cambio de modelo** - Cambia de modelo de forma flexible en la misma interfaz según la tarea
- 📄 **Vista previa en tiempo real** - Vista previa visual de 9+ formatos, resultados al instante
- 💾 **Seguridad de datos local** - Todas las conversaciones y archivos se guardan localmente, los datos no salen de tu dispositivo

---

### ❓ Preguntas rápidas

<details>
<summary><strong>Q: ¿Por qué Foundry es un gran reemplazo de Claude Cowork?</strong></summary>
A: Foundry es un **Multi-AI Agent Desktop** **gratuito y de código abierto**. En comparación con el Cowork oficial que solo funciona en macOS y está bloqueado a Claude, Foundry es su **versión mejorada multi-modelo y multiplataforma**, con cobertura profunda de **escenarios de automatización de oficina con IA**.

| Dimensión   | Claude Cowork        | Foundry (este proyecto)                     |
| :---------- | :------------------- | :------------------------------------------ |
| SO          | Solo macOS           | 🍏 macOS / 🪟 Windows / 🐧 Linux            |
| Modelos     | Solo Claude          | 🤖 Gemini, Claude, DeepSeek, OpenAI, Ollama |
| Interacción | GUI                  | 🖥️ GUI completo + acceso remoto WebUI       |
| Costo       | Suscripción $100/mes | 🆓 Completamente gratis y open source       |

**Soporte profundo para escenarios de oficina con IA:**

- **Gestión de archivos**: Organiza carpetas locales desordenadas y renombra en lote con un clic.
- **Procesamiento de datos**: Analiza en profundidad y embellece automáticamente reportes de Excel.
- **Generación de documentos**: Escribe y da formato automáticamente a PPT, Word y Markdown.
- **Vista previa instantánea**: Paneles de vista previa integrados para más de 9 formatos, resultados visibles al instante.
</details>

<details>
<summary><strong>Q: ¿Qué puedo hacer con Foundry?</strong></summary>
A: Puede ser tu **espacio de trabajo Cowork privado**. Te ayuda a organizar carpetas por lotes, embellecer Excel en profundidad y previsualizar código web en tiempo real. Es la mejor opción gráfica para explorar flujos de automatización de oficina y mejorar tu experiencia con Claude Code o Gemini CLI.
</details>

<details>
<summary><strong>Q: ¿Foundry está listo para usar desde el primer momento?</strong></summary>
A: ¡Sí! Foundry está listo justo después de la instalación con Gemini CLI integrado, sin instalaciones adicionales. Si ya tienes Gemini CLI u otras herramientas de línea de comandos, Foundry las detecta automáticamente para ofrecer una experiencia más rica.
</details>

<details>
<summary><strong>Q: ¿Es gratis?</strong></summary>
A: Foundry es completamente gratuito y de código abierto, pero el uso de modelos de IA requiere las API Keys correspondientes.
</details>

<details>
<summary><strong>Q: ¿Qué modelos de IA son compatibles?</strong></summary>
A: Compatible con modelos principales como Gemini, OpenAI, Claude, Qwen, así como modelos locales como Ollama y LM Studio.

También puedes ejecutar varios agentes de IA simultáneamente (como Gemini CLI, Claude Code, Qwen Code, etc.); consulta la guía de configuración para más detalles.

</details>

<details>
<summary><strong>Q: ¿Mis datos están seguros?</strong></summary>
A: Todos los datos de conversación se almacenan en una base de datos SQLite local y no se suben a ningún servidor.
</details>

---

## 🎬 Mira cómo la gente usa Foundry

_Mira cómo los creadores de contenido reseñan y usan Foundry_

<p align="center">
  <a href="https://www.youtube.com/watch?v=yUU5E-U5B3M" target="_blank">
    <img src="https://img.youtube.com/vi/yUU5E-U5B3M/maxresdefault.jpg" alt="WorldofAI Reseña" width="400">
  </a>
  &nbsp;&nbsp;
  <a href="https://www.youtube.com/watch?v=enQnkKfth10" target="_blank">
    <img src="https://img.youtube.com/vi/enQnkKfth10/maxresdefault.jpg" alt="Julian Goldie SEO Reseña" width="400">
  </a>
</p>
<p align="center">
  <em>🎬 WorldofAI (200K suscriptores)</em> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <em>🎬 Julian Goldie SEO (318K suscriptores)</em>
</p>

> 💡 **¿Has hecho un video sobre Foundry?** [Cuéntanos en X](https://x.com/Foundry) y lo destacaremos aquí. Valoramos tus comentarios y seguiremos mejorando.

---

## 📖 Guía de uso detallada

<details>
<summary><strong>📖 Desplegar para ver la guía completa</strong></summary>

### 🚀 Inicio rápido

- [📖 Guía completa de instalación](https://github.com/Foundry/Foundry/wiki/Getting-Started) - Pasos detallados desde la descarga hasta la configuración
- [⚙️ Guía de configuración de LLM](https://github.com/Foundry/Foundry/wiki/LLM-Configuration) - Configuración de modelos de IA en múltiples plataformas
- [🤖 Configuración del modo multiagente](https://github.com/Foundry/Foundry/wiki/ACP-Setup) - Integración de agentes IA de terminal
- [🔌 Configuración de herramientas MCP](https://github.com/Foundry/Foundry/wiki/MCP-Configuration-Guide) - Configuración del servidor del protocolo de contexto de modelos
- [🎨 Configuración de generación de imágenes](https://github.com/Foundry/Foundry/wiki/Foundry-Image-Generation-Tool-Model-Configuration-Guide) - Tutorial de configuración de generación de imágenes con IA
- [🌐 Guía de configuración de WebUI](https://github.com/Foundry/Foundry/wiki/WebUI-Configuration-Guide) - Tutorial completo de configuración de WebUI

### 🎯 Casos de uso

- [📁 Gestión de archivos](https://github.com/Foundry/Foundry/wiki/file-management) - Organización inteligente de archivos
- [📊 Procesamiento de Excel](https://github.com/Foundry/Foundry/wiki/excel-processing) - Procesamiento de datos impulsado por IA
- [🎨 Generación de imágenes](https://github.com/Foundry/Foundry/wiki/Foundry-Image-Generation-Tool-Model-Configuration-Guide) - Creación de imágenes con IA
- [📚 Más casos de uso](https://github.com/Foundry/Foundry/wiki/Use-Cases-Overview)
  - [🎬 WorldofAI](https://www.youtube.com/watch?v=yUU5E-U5B3M) - Reseña en YouTube
  - [🎬 Julian Goldie SEO](https://www.youtube.com/watch?v=enQnkKfth10) - Reseña en YouTube

### ❓ Soporte y ayuda

- [❓ FAQ](https://github.com/Foundry/Foundry/wiki/FAQ) - Preguntas y solución de problemas
- [🔧 Tutoriales de configuración y uso](https://github.com/Foundry/Foundry/wiki/Configuration-Guides) - Documentación completa de configuración

</details>

---

## 🚀 Inicio rápido

### 💻 Requisitos del sistema

- **macOS**: 10.15 o superior
- **Windows**: Windows 10 o superior
- **Linux**: Ubuntu 18.04+ / Debian 10+ / Fedora 32+
- **Memoria**: Recomendado 4GB o más
- **Almacenamiento**: Al menos 500MB de espacio disponible

### 📥 Descargar

<p>
  <a href="https://github.com/Foundry/Foundry/releases">
    <img src="https://img.shields.io/badge/Descargar-Última%20versión-32CD32?style=for-the-badge&logo=github&logoColor=white" alt="Download Latest Release" height="50">
  </a>
</p>

### 🍺 Instalar vía Homebrew (macOS)

```bash
brew install foundry
```

### 🔧 Instalación simple

1. **Descargar e instalar** la aplicación Foundry
2. **Configurar el servicio de IA** - Compatible con inicio de sesión de Google o autenticación por API Key
3. **Empezar a usar** - Experimenta de inmediato una interfaz moderna de chat con IA

> 💡 **¿Necesitas una guía de configuración detallada?** Consulta nuestro [Tutorial completo de instalación](https://github.com/Foundry/Foundry/wiki/Getting-Started)

---

## 🤝 Comunidad y soporte

### 💬 Comunidad

**💡 ¡Tus ideas importan!** Valoramos las sugerencias y comentarios de cada usuario. Ya sea sobre ideas de funciones, experiencia de uso o problemas que encuentres, ¡contáctanos cuando quieras!

<p align="center">
  <a href="https://x.com/Foundry" target="_blank">
    <img src="./resources/contactus-x.png" alt="Contact Us on X" width="600">
  </a>
</p>

- [💬 GitHub Discussions](https://github.com/Foundry/Foundry/discussions) - **Comparte ideas, sugiere mejoras, intercambia consejos de uso**
- [🐛 Reportar problemas](https://github.com/Foundry/Foundry/issues) - Informar errores o solicitar funciones
- [📦 Actualizaciones de versiones](https://github.com/Foundry/Foundry/releases) - Obtener la última versión
- [💬 Discord Community](https://discord.gg/2QAwJn7Egx) - **Únete a nuestra comunidad en inglés**
- [💬 WeChat (中文)](./resources/wx.png) - **Haz clic para ver el QR**

### 🤝 Contribuir

¡Bienvenido a enviar Issues y Pull Requests!

1. Haz fork de este proyecto
2. Crea una rama de función (`git checkout -b feature/AmazingFeature`)
3. Confirma tus cambios (`git commit -m 'Add some AmazingFeature'`)
4. Empuja la rama (`git push origin feature/AmazingFeature`)
5. Abre un Pull Request

---

## 📄 Licencia

Este proyecto está licenciado bajo [Apache-2.0](LICENSE).

---

## 👥 Contribuidores

¡Gracias a todos los desarrolladores que han contribuido a Foundry!

<p align="center">
  <a href="https://github.com/Foundry/Foundry/graphs/contributors">
    <img src="https://contrib.rocks/image?repo=Foundry/Foundry&max=50" alt="Contributors" />
  </a>
</p>

## 📊 Historial de estrellas

<p align="center">
  <a href="https://www.star-history.com/#Foundry/foundry&Date" target="_blank">
    <img src="https://api.star-history.com/svg?repos=Foundry/foundry&type=Date" alt="GitHub Star Trends" width="600">
  </a>
</p>

<div align="center">

**⭐ Si te gusta, danos una estrella**

[Reportar bug](https://github.com/Foundry/Foundry/issues) · [Solicitar función](https://github.com/Foundry/Foundry/issues)

</div>
