...
function isProQuotaExceededError(error: unknown): boolean {
  ...
  // Check if error contains Pro model related quota error message
...
function isGenericQuotaExceededError(error: unknown): boolean {
  ...
  // Check if error contains quota related error
  const errorStr = JSON.stringify(error).toLowerCase();
...
function getRateLimitMessage(authType?: AuthType, error?: unknown, userTier?: UserTierId, currentModel?: string, fallbackModel?: string): string {
  switch (authType) {
