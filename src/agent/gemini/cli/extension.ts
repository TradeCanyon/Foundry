...
function loadExtension(extensionDir: string): GeminiCLIExtension | null {
  ...
  try {
    ...
    return {
      ...
      isActive: true, // Default active, later adjusted by annotateActiveExtensions
      path: extensionDir,
      contextFiles,
...
export function annotateActiveExtensions(extensions: GeminiCLIExtension[], enabledExtensionNames: string[]): GeminiCLIExtension[] {
  // If no enabled list specified, all extensions are active
  ...
  // If 'none' is specified, disable all extensions
