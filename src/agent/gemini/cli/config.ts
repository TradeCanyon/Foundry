...
export async function loadCliConfig({ workspace, settings, extensions, sessionId, proxy, model, conversationToolConfig, yoloMode, mcpServers, skillsDir, enabledSkills }: LoadCliConfigOptions): Promise<Config> {
  ...
  const builtinSkillsExtension: GeminiCLIExtension = {
    name: 'foundry-builtin-skills',
    version: '1.0.0',
    isActive: true,
    path: skillsDir || '',
    contextFiles: [],
    id: 'foundry-builtin-skills',
  ...
  // Use conversation-level tool configuration
    ...
    if (allowedNames.size > 0) {
      mcpServersConfig = Object.fromEntries(
        Object.entries(mcpServersConfig).filter(([key, server]) => {
          ...
          if (!isAllowed) {
            // aioncli-core v0.18.4: use server.extension?.name instead of server.extensionName
  ...
  const config = new Config({
    ...
    // Foundry is a desktop app with user interaction support, needs interactive: true
    interactive: true,
    ...
    // Enabled when skillsDir has value
  ...
  //
  // Workflow:
  // 1. handler calls apiKeyManager.rotateKey() to update API Key in process.env
  // 2. aioncli-core's tryRotateApiKey detects env change and calls config.refreshAuth()
  // 3. Return 'retry_once' for one-time retry, 'stop' to stop retrying
  //
...
function mergeMcpServers(settings: Settings, extensions: GeminiCLIExtension[], uiMcpServers?: Record<string, unknown>) {
  ...
  // Add MCP servers from UI configuration (highest priority)
  if (uiMcpServers) {
