...
export const processGeminiStreamEvents = async (stream: AsyncIterable<ServerGeminiStreamEvent>, config: Config, onStreamEvent: (event: { type: ServerGeminiStreamEvent['type']; data: unknown }) => void, monitorOptions?: StreamMonitorOptions): Promise<StreamProcessingStatus> => {
  ...
  try {
    for await (const event of stream) {
      // Record event received, update heartbeat time
      monitor.recordEvent();

      // Check if heartbeat timeout (no data for long time)
      if (monitor.isHeartbeatTimeout()) {
        console.warn('[StreamMonitor] Stream heartbeat timeout, connection may be stale');
        // Don't interrupt immediately, let upper layer decide
      }
      ...
      switch (event.type) {
        ...
        // Use string literal for compatibility with older aioncli-core versions
        ...
        case ServerGeminiEventType.Finished:
          {
            // Pass Finished event, contains token usage statistics
            onStreamEvent({ type: event.type, data: (event as unknown as { value: unknown }).value });
          }
          break;
 ...
 *
 * Improvements:
 * 1. Use globalToolCallGuard to protect ongoing tool calls
 * 2. Protected tool calls won't be misjudged as cancelled
 * 3. Automatically remove protection after tool completes
 */
