...
/**
 * CodexSessionManager - Session management capabilities (based on ACP pattern)
 * Provides unified connection state management, session lifecycle and status notifications
 */
// Global state management, ensure all Codex sessions share state
...
export class CodexSessionManager {
  ...
  /**
   * Execute connection sequence - based on ACP's connection flow
   */
  ...
  private performAuthentication(): Promise<void> {
    ...
    // Currently Codex handles authentication through the CLI itself
  ...
  private setStatus(status: CodexSessionStatus): void {
    this.status = status;
    // Just update local state, global ID ensures uniqueness
