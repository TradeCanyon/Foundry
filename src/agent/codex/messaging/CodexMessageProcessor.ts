...
export class CodexMessageProcessor {
  ...
  handleReasoningMessage(msg: Extract<CodexEventMsg, { type: 'agent_reasoning_delta' }> | Extract<CodexEventMsg, { type: 'agent_reasoning' }> | Extract<CodexEventMsg, { type: 'agent_reasoning_section_break' }>) {
    ...
    // AGENT_REASONING_SECTION_BREAK doesn't add content, just resets current reasoning
  ...
  processStreamError(message: string) {
    ...
    let msgId: string;
    if (isRetryMessage) {
      // All retry messages use the same ID so they get merged/updated
      msgId = `stream_retry_${errorHash}`;
    } else if (isFinalError) {
      // Final error message also uses retry message ID so it replaces retry messages
      msgId = `stream_retry_${errorHash}`;
    } else {
  ...
  private normalizeRetryMessage(message: string): string {
    // For retry messages, extract core error info, ignoring retry count and delay time
    ...
    // Other error message types return directly
    return message;
  }
