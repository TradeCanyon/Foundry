...
export class CodexAgent {
  ...
  async newSession(cwd?: string, initialPrompt?: string): Promise<{ sessionId: string }> {
    ...
    const args: Record<string, unknown> = {
      ...
      sandbox: this.sandboxMode, // Force sandbox mode
    };
    ...
    // Return session ID, let subsequent flow continue
    return { sessionId: convId };
  }
  ...
  async sendPrompt(prompt: string): Promise<void> {
    ...
    try {
      await this.conn?.request(
        ...
        600000 // 10 minute timeout, avoid long task interruption
      );
    } catch (error) {
      ...
      if (errorMsg.includes('timed out')) {
        // Don't throw error since reasoning_delta events are still arriving normally
        return;
      }
    ...
    try {
      if (error.type === 'network') {
        ...
        if (this.onNetworkError) {
          ...
          // Network errors also go through stream error handling
