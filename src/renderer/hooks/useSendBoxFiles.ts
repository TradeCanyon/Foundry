 ...
 * Supports functional updates to avoid closure pitfalls
 */
...
/**
 * Standalone file formatting utility function for components like GUID that don't need full SendBox state management
 ...
 * Eliminates code duplication between ACP, Gemini, and GUID components
 */
export const useSendBoxFiles = ({ atPath, uploadFile, setAtPath, setUploadFile }: UseSendBoxFilesProps) => {
  // Handle dragged or pasted files
  const handleFilesAdded = useCallback(
    (files: FileMetadata[]) => {
      const filePaths = files.map((file) => file.path);
      // Use functional update based on latest state, not closure state
