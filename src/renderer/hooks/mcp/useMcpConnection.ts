 ...
 * Handles connection testing and status updates for MCP servers
 */
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          if (result.success) {
            ...
            // Don't modify enabled field on successful connection, let user decide whether to install
            ...
            // Connection test successful, no additional operations
          } else {
            // Update server status to error, and disable installation
            // Auto-set enabled=false on connection failure to avoid installing failed services
