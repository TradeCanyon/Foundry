    ...
    async (response: McpOperationResponse, operation: 'sync' | 'remove', successMessage?: string, skipRecheck = false) => {
      if (response.success && response.data) {
        ...
        if (failedAgents.length > 0) {
          ...
          // No longer show "operation started" message as it was already shown at operation start
        }
        ...
        if (!skipRecheck) {
          void ConfigStorage.get('mcp.config')
            .then((latestServers) => {
              if (latestServers) {
                // Can trigger status check here, but callback needs to be provided at usage site
              }
            })
            .catch(() => {
