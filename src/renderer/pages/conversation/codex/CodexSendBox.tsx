...
const CodexSendBox: React.FC<{ conversation_id: string }> = ({ conversation_id }) => {
  ...
  // When conversation ID changes, clear all states to avoid state pollution
  useEffect(() => {
  ...
  useEffect(() => {
    processingContext?.setIsProcessing(isProcessing);
  }, [isProcessing, processingContext]);
  ...
  const onSendHandler = async (message: string) => {
    const msg_id = uuid();
    // Clear input and selected files immediately for better UX
    setContent('');
    ...
    // No longer auto-adding @ prefix to avoid message newline and ambiguity
    ...
    // Frontend writes user message first to avoid navigation/event race causing message not visible
    ...
    addOrUpdateMessage(userMessage, true); // Save to storage immediately to avoid loss on refresh
    setAiProcessing(true);
    try {
      ...
      await ipcBridge.codexConversation.sendMessage.invoke({
        ...
        msg_id,
        conversation_id,
        files: [...currentUploadFile, ...atPathStrings], // Includes uploaded files and selected workspace files
      });
  ...
  // Handle initial message from guid page, wait for connection state to be established before sending
  useEffect(() => {
    if (!conversation_id || !codexStatus) return;

    // Only send initial message when connection state is session_active
    ...
    const processInitialMessage = async () => {
      ...
      try {
        ...
        // Clear processing flag on send failure to allow retry
    ...
    // Small delay to ensure status message has been fully processed
