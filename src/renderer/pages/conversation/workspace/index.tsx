...
const ChatWorkspace: React.FC<WorkspaceProps> = ({ conversation_id, workspace, eventPrefix = 'gemini', messageApi: externalMessageApi }) => {
  ...
  // Workspace tree collapse state - global unified collapse state
  ...
  const [isWorkspaceCollapsed, setIsWorkspaceCollapsed] = useState(() => {
    try {
      ...
      if (stored) {
        // Store boolean value directly, not per workspace path
        return stored === 'true';
      }
    } catch {
      // Ignore errors
  ...
  // Persist collapse state - global unified
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    // Do not focus on first render or conversation switch
  ...
  return (
    <>
      ...
      >
        ...
        {/* Toolbar */}
        <div className='px-12px'>
          {/* Border divider - below search box */}
