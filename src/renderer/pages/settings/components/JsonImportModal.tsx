...
const JsonImportModal: React.FC<JsonImportModalProps> = ({ visible, server, onCancel, onSubmit, onBatchImport }) => {
  ...
  React.useEffect(() => {
    if (visible && server) {
      ...
      if (server.originalJson) {
        ...
        // For legacy data without originalJson, generate JSON config
        const serverConfig = {
          mcpServers: {
            [server.name]: {
  ...
  const handleSubmit = () => {
    // Syntax validation already passed (button disable logic ensures this), parse directly
    ...
    // Use batch import if there are multiple servers
    if (serverKeys.length > 1 && onBatchImport) {
      const serversToImport = serverKeys.map((serverKey) => {
        ...
        return {
          ...
          tools: [] as IMcpTool[], // Initialize as empty array when importing via JSON, can be populated via connection test later
