...
const hasBuiltinSkills = (assistantId: string): boolean => {
  ...
  // Either defaultEnabledSkills or skillFiles config is sufficient
...
const AssistantManagement: React.FC<AssistantManagementProps> = ({ message }) => {
  ...
  const handleEdit = async (assistant: AcpBackendConfig) => {
    ...
    // Load rules, skills content first
    try {
  ...
  const handleSave = async () => {
    try {
      ...
      if (pendingSkills.length > 0) {
        // Filter skills that truly need to be imported (not in availableSkills)
