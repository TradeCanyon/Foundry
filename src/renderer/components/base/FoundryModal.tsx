...
export interface ModalContentStyleConfig {
  /** Background color, defaults to var(--bg-1) */
  background?: string;
...
/** FoundryModal component Props */
export interface FoundryModalProps extends Omit<ModalProps, 'title' | 'footer'> {
  children?: React.ReactNode;

  /** Preset size, will be overridden by width/height in style */
  size?: ModalSize;

  /** Header configuration, can be a simple title string or full config object */
 ...
 * ```tsx
 * // Basic usage
 * <FoundryModal visible={true} onCancel={handleClose} header="Title">
 *   Content
 * </FoundryModal>
 *
 * // Preset size
 * <FoundryModal visible={true} size="large" header="Large Modal">
 *   Content
 * </FoundryModal>
 *
 * // Custom header
...
const FoundryModal: React.FC<FoundryModalProps> = ({
  children,
  size,
  header,
  footer,
  ...
  // Process contentStyle configuration, convert to CSS variables
  ...
  // Process Header configuration (backward compatible)
  const headerConfig: ModalHeaderConfig = React.useMemo(() => {
    // If using new header configuration
    if (header !== undefined) {
      // If it's a string or ReactNode, convert to title configuration
  ...
  const renderHeader = () => {
    ...
    // If no title and close button not shown, don't render header
  ...
  return (
    <Modal {...props} title={null} closable={false} footer={null} onCancel={onCancel} className={`foundry-modal ${className}`} style={finalStyle} getPopupContainer={() => document.body}>
      <div className='foundry-modal-wrapper' style={{ borderRadius: borderRadiusVal }}>
        {renderHeader()}
        <div className='foundry-modal-body-content' style={bodyInlineStyle}>
          {children}
        </div>
        {renderFooter()}
      </div>
...
FoundryModal.displayName = 'FoundryModal';
