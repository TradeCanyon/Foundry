...
const WebuiModalContent: React.FC = () => {
  ...
  const handleAllowRemoteChange = async (checked: boolean) => {
    ...
    if (wasRunning) {
      ...
      try {
        ...
        // Restart immediately (server stops quickly)
  ...
  return (
    <div className='flex flex-col h-full w-full'>
            ...
            <PreferenceRow
              ...
              description={
                <>
                  ...
                  {'  '}
                  <button className='text-primary hover:underline cursor-pointer bg-transparent border-none p-0 text-12px' onClick={() => shell.openExternal.invoke('https://github.com/Foundry/Foundry/wiki/Remote-Internet-Access-Guide').catch(console.error)}>
      ...
      {/* Set New Password Modal */}
      <FoundryModal visible={setPasswordModalVisible} onCancel={() => setSetPasswordModalVisible(false)} onOk={handleSetNewPassword} confirmLoading={passwordLoading} title={t('settings.webui.setNewPassword')} size='small'>
