...
const HorizontalFileList: React.FC<HorizontalFileListProps> = ({ children }) => {
   ...
   * Calculate whether the container is scrollable and if it's at the start/end position
   */
  const checkScroll = React.useCallback(() => {
    ...
    // Check if at start position (left)
    const isAtStart = container.scrollLeft <= 1;
    // Check if at end position (right)
  ...
  useEffect(() => {
    ...
    // Use ResizeObserver to monitor container size changes and auto-update scroll state
    // ResizeObserver handles most layout changes well
    const resizeObserver = new ResizeObserver(scheduleCheck);
    resizeObserver.observe(container);

    // Listen to scroll events to update button display state in real-time
  ...
  // Also check once when children change, but not as a useEffect dependency to avoid frequent re-runs
  useEffect(() => {
    checkScroll();
