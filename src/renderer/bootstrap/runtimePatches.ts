...
// Silence React 19 ref deprecation warnings from third-party libraries (waiting for library updates)
...
const patchGlobalErrorListeners = () => {
  ...
  // Webpack/Arco overlays run (avoid being flooded by ResizeObserver warnings before overlay triggers, while preserving real errors).
...
const patchResizeObserver = () => {
  ...
  // browsers treat as "ResizeObserver loop" (execute callback on next frame to completely avoid ResizeObserver loop limit warnings).
...
const patchGlobalErrorFilters = () => {
  ...
  // (globally filter ResizeObserver loop warnings, only ignore whitelisted messages, other errors still propagate).
...
const patchConsole = () => {
  // Console patch mirrors the listener filters so devtools logs stay clean (console also intercepts to prevent being flooded by repeated warnings).
