...
class PasteServiceClass {
  ...
  async handlePaste(event: React.ClipboardEvent | ClipboardEvent, supportedExts: string[], onFilesAdded: (files: FileMetadata[]) => void, onTextPaste?: (text: string) => void): Promise<boolean> {
    // Stop event bubbling immediately to avoid global listener duplicate processing
    event.stopPropagation();
    ...
    if (files && files.length > 0) {
      // Process files, skip text processing
      const fileList: FileMetadata[] = [];
      for (let i = 0; i < files.length; i++) {
        const file = files[i];
        const filePath = (file as File & { path?: string }).path;

        // Check if file has path (File objects in Electron have additional path property)

        if (!filePath && file.type.startsWith('image/')) {
          // Clipboard image, need to check if type is supported
          ...
          if (allowAll || supportedExts.includes(fileExt)) {
            try {
              ...
              // Generate concise filename; if clipboard image has odd default name, replace with concise name
              const now = new Date();
          ...
          // File with path (dragged from file manager)
          ...
          // Non-image file without path (copied from file manager)
          const fileExt = getFileExtension(file.name);

          if (allowAll || supportedExts.includes(fileExt)) {
            // For copy-pasted files, we need to create temporary files
            try {
              const arrayBuffer = await file.arrayBuffer();
              const uint8Array = new Uint8Array(arrayBuffer);
    ...
    if (clipboardText && (!files || files.length === 0)) {
      // On iOS, let Safari handle plain text paste to avoid paste menu/keyboard jitter issues
      const isIOS = typeof navigator !== 'undefined' && /iP(hone|ad|od)/.test(navigator.userAgent);
      if (isIOS) {
        return false;
      }
      if (onTextPaste) {
        // Clean up extra newlines in text, especially trailing newlines
