 ...
 * @returns Relative file path, or null if unable to parse
 */
export function parseFilePathFromDiff(diffContent: string): string | null {
  ...
  // Try git diff format (+++ b/ takes priority as it points to new file)
  for (const line of lines) {
 ...
 *
 * @param diffContent Diff content
 * @returns Extracted clean file content
 */
