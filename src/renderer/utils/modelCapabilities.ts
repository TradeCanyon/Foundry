...
/**
 * Regular expressions for capability matching - inspired by Cherry Studio's approach
 */
const CAPABILITY_PATTERNS: Record<ModelType, RegExp> = {
  text: /gpt|claude|gemini|qwen|llama|mistral|deepseek|o1|o3|o4/i,
  vision: /4o|gpt-5|claude|gemini-.*-pro|gemini-.*-flash|qwen-vl|llava|vision/i,
  function_calling: /gpt-4|gpt-5|claude|gemini|qwen|deepseek|o3|o4/i,
  image_generation: /flux|diffusion|stabilityai|sd-|dall|cogview|janus|midjourney|mj-|imagen/i,
  web_search: /search|perplexity/i,
  reasoning: /o1|o3|o4|reasoning|think/i,
  ...
  excludeFromPrimary: /dall-e|flux|stable-diffusion|midjourney|flash-image|image|embed|rerank/i, // Models to exclude from primary selection
};
...
const CAPABILITY_EXCLUSIONS: Record<ModelType, RegExp[]> = {
  ...
  function_calling: [/aqa(?:-[\\w-]+)?/i, /imagen(?:-[\\w-]+)?/i, /dall-e/i, /embed/i, /rerank/i],
  image_generation: [],
  web_search: [],
  reasoning: [],
  embedding: [],
...
/**
 * Get the lowercase base version of the model name (for matching)
...
/**
 * Get capability rule for a specific provider
...
/**
 * Determine if a model has a specific capability - inspired by Cherry Studio's three-tier logic
 * @param model - Model object
 * @param type - Capability type
...
export const hasModelCapability = (model: IProvider, type: ModelType): boolean | undefined => {
  ...
  if (userSelected !== undefined) {
    ...
    if (providerRule !== null) {
      ...
      // Check if any model under the platform supports this capability
      const modelNames = model.model || [];

      // Unified logic for all capability types
...
/**
 * Determine if a specific model under a platform has a capability
