 ...
 * Note: iFlow CLI supports stdio, SSE, HTTP transport types, supports headers, does not support streamable_http
 */
export class IflowMcpAgent extends AbstractMcpAgent {
  ...
  /**
   * Detect iFlow CLI's MCP configuration (internal implementation, no lock)
   */
  private async detectMcpServersInternal(_cliPath?: string): Promise<IMcpServer[]> {
    try {
      ...
      for (const line of lines) {
        ...
        // Look for format: "checkmark Name: command (transport) - Status" (supports both English and Chinese status)
  ...
  /**
   * Detect iFlow CLI's MCP configuration (public interface, uses lock)
   */
  ...
  installMcpServers(mcpServers: IMcpServer[]): Promise<McpOperationResult> {
    const installOperation = async () => {
      try {
        // Get current iFlow MCP server list (use internal method to avoid deadlock)
        ...
        for (const server of mcpServers.filter((s) => s.enabled)) {
          ...
          try {
            ...
            // Continue processing other servers, don't stop entire process due to one failure
          }
        }
