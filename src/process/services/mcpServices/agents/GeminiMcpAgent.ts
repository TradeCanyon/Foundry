 ...
 *
 * Uses Google's official Gemini CLI mcp subcommand to manage MCP server configuration
...
export class GeminiMcpAgent extends AbstractMcpAgent {
  ...
  /**
   * Detect Google Gemini CLI's MCP configuration
   */
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          if (attempt === 1) {
            ...
            // If not first attempt, add short delay to avoid conflict with other operations
          ...
          for (const line of lines) {
            ...
            // Look for format like: "✓ 12306-mcp: npx -y 12306-mcp (stdio) - Connected"
          ...
          // Validate result: if output contains "Configured MCP servers:" but no servers detected, may be truncated
          ...
          // If there are retry opportunities, continue to next attempt
  ...
  removeMcpServer(mcpServerName: string): Promise<McpOperationResult> {
    const removeOperation = async () => {
      try {
        // Use Gemini CLI command to delete MCP server
        ...
        try {
          ...
          try {
            ...
            if (result.stdout && result.stdout.includes('removed')) {
              ...
              // Server doesn't exist, also consider successful
              return { success: true };
            }
          } catch (projectError) {
            // If server doesn't exist, also consider successful
