 ...
 *
 * Uses Codex CLI's mcp subcommand to manage MCP server configuration
...
export class CodexMcpAgent extends AbstractMcpAgent {
    ...
    const detectOperation = async () => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          // Parse args (if "-" means no arguments)
          const args = argsStr === '-' ? [] : argsStr.split(/\s+/);

          // Parse env (if "-" means no environment variables)
          const env: Record<string, string> = {};
          if (envStr && envStr !== '-') {
            // Environment variable format may be KEY=VALUE form
  ...
  removeMcpServer(mcpServerName: string): Promise<McpOperationResult> {
    const removeOperation = async () => {
      try {
        // Use Codex CLI command to delete MCP server
        ...
        try {
          ...
          if (result.stdout && (result.stdout.includes('removed') || result.stdout.includes('Removed'))) {
            ...
            // Other cases considered successful (backward compatibility)
            return { success: true };
          }
        } catch (cmdError) {
