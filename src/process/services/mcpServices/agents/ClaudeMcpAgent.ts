...
export class ClaudeMcpAgent extends AbstractMcpAgent {
  ...
  detectMcpServers(_cliPath?: string): Promise<IMcpServer[]> {
    const detectOperation = async () => {
      try {
        ...
        // If no MCP servers configured, return empty array
        ...
        for (const line of lines) {
          // Remove ANSI color codes (support multiple formats)
          ...
          // Look for format like: "12306-mcp: npx -y 12306-mcp - ✓ Connected" or "12306-mcp: npx -y 12306-mcp - ✗ Failed to connect"
          // Support multiple status texts
      ...
      try {
        for (const server of mcpServers) {
          if (server.transport.type === 'stdio') {
            // Use Claude Code CLI to add MCP server to user scope (global config)
            // Foundry is a global tool, MCP configuration should be available to all projects
            ...
            if (server.transport.args?.length || Object.keys(server.transport.env || {}).length) {
              ...
              if (server.transport.args?.length) {
                // Quote each argument appropriately to prevent args with special chars from being misinterpreted
                const quotedArgs = server.transport.args.map((arg: string) => `"${arg}"`).join(' ');
                command += ` ${quotedArgs}`;
              }
            }

            // Environment variables added after --
            if (envArgs) {
              command += ` ${envArgs}`;
            }
            ...
            try {
              ...
              // Continue processing other servers, don't stop due to one failure
            }
          } else {
  ...
  removeMcpServer(mcpServerName: string): Promise<McpOperationResult> {
    const removeOperation = async () => {
      try {
        // Use Claude CLI command to delete MCP server (try different scopes)
        // Try in order: user (Foundry default) -> local -> project
        // User scope takes priority because Foundry installs with user scope
        ...
        for (const scope of scopes) {
          try {
            ...
            // If no "removed" message but also no error, server may not exist in this scope
            // Continue trying next scope
          } catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);

            // If "not found" error, continue trying next scope
        ...
        // If all scopes tried, consider deletion successful (server may not have existed)
