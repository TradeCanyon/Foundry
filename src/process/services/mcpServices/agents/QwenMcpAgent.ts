    ...
    const detectOperation = async () => {
      try {
        ...
        for (const line of lines) {
          ...
          // Look for format like: "✓ filesystem: npx @modelcontextprotocol/server-filesystem /path (stdio) - Connected"
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            // CLI commands all failed, try direct config file operation as fallback
            const configPath = join(homedir(), '.qwen', 'client_config.json');

            if (!existsSync(configPath)) {
              return { success: true }; // Config file doesn't exist, consider already deleted
            }
            ...
            try {
              ...
              return { success: true }; // If config file operation fails, still consider successful
            }
          }
        }
      } catch (error) {
