...
/**
 * MCP source type - includes all ACP backends and Foundry built-in
 */
export type McpSource = AcpBackendAll | 'foundry';
...
export abstract class AbstractMcpAgent implements IMcpProtocol {
  ...
  /**
   * Mutex lock to ensure operations are executed serially
   */
  protected withLock<T>(operation: () => Promise<T>): Promise<T> {
    ...
    const newOperation = currentQueue
      ...
      .catch((error) => {
        ...
        // Continue executing the next operation in the queue even if the current one fails
        throw error;
      });

    // Update the queue (ignore errors to ensure the queue continues)
  ...
  /**
   * Common implementation for testing MCP server connection
  ...
  /**
   * Common implementation for testing Stdio connection
  ...
  protected async testStdioConnection(
    ...
    try {
      ...
      // Connect to server and get tool list
      ...
      // Detect npm cache issues and auto-fix
  ...
  /**
   * Common implementation for testing SSE connection
   * Uses MCP SDK for proper protocol communication
   */
  ...
  /**
   * Common implementation for testing HTTP connection
   */
  ...
  /**
   * Common implementation for testing Streamable HTTP connection
   * Uses MCP SDK for proper protocol communication
   */
