...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'foundry');
};
...
/**
 * Get data path, using CLI-safe symlink (~/.foundry) on macOS.
 */
export const getDataPath = (): string => {
  const rootPath = app.getPath('userData');
  const dataPath = path.join(rootPath, 'foundry');
  return ensureCliSafeSymlink(dataPath, '.foundry');
};

/**
 * Get config path, using CLI-safe symlink (~/.foundry-config) on macOS.
 */
export const getConfigPath = (): string => {
  const rootPath = app.getPath('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, '.foundry-config');
};
...
export const generateHashWithFullName = (fullName: string): string => {
  ...
  // Take absolute value, convert to hex, and pad to 8 characters
 ...
 * Note: Includes path validation to prevent copying to self or subdirectory causing infinite recursion (fixes cache directory loop bug on Windows)
 */
...
export async function copyDirectoryRecursively(src: string, dest: string, options: CopyOptions = {}) {
  ...
  // Normalize paths: Windows to lowercase (case-insensitive), Unix/macOS as-is (case-sensitive)
  ...
  // Prevent copying to subdirectory (F:\code -> F:\code\cache) - causes infinite recursion
...
// Verify if two directories have the same file name structure
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[Foundry] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (dir: string, files?: string[], skipCleanup = false): Promise<string[]> => {
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[Foundry] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[Foundry] Original path: ${file}`);
      // Skip non-existent files instead of throwing error
      continue;
    }
    ...
    try {
      ...
      console.error(`[Foundry] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
