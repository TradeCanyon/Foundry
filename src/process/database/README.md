# Foundry Database System

本文档介绍 Foundry 的新数据库系统，它使用 **better-sqlite3** (主进程) 作为持久化存储。
...
- **数据库文件**: `{userData}/config/foundry.db`
- **图片文件**: `{userData}/data/images/`

其中 `{userData}` 为：

- macOS: `~/Library/Application Support/Foundry/`
- Windows: `%APPDATA%/Foundry/`
- Linux: `~/.config/Foundry/`
...
直接复制 `foundry.db` 和 `foundry.db-wal` 文件即可。

## API 参考

### FoundryDatabase 主要方法
