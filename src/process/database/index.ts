...
/**
 * Main database class for Foundry
...
export class FoundryDatabase {
  ...
  constructor() {
    const finalPath = path.join(getDataPath(), 'foundry.db');
   ...
   */
  getLatestConversationBySource(source: 'foundry' | 'telegram', userId?: string): IQueryResult<TChatConversation | null> {
    try {
  ...
  /**
   * Delete all messages after a specific message (by created_at timestamp)
   * Used for message editing - deletes subsequent messages when user edits an earlier message
   */
  deleteMessagesAfter(conversationId: string, afterTimestamp: number): IQueryResult<number> {
    try {
      const stmt = this.db.prepare('DELETE FROM messages WHERE conversation_id = ? AND created_at > ?');
      const result = stmt.run(conversationId, afterTimestamp);
   ...
   * Used for message editing. Wraps content in JSON format for text messages.
   */
  updateMessageContent(messageId: string, content: string): IQueryResult<boolean> {
    try {
      // Content is stored as JSON in the database
      // For text messages, the format is { content: "..." }
      const jsonContent = JSON.stringify({ content });
      const stmt = this.db.prepare('UPDATE messages SET content = ? WHERE id = ?');
      const result = stmt.run(jsonContent, messageId);
...
let dbInstance: FoundryDatabase | null = null;

export function getDatabase(): FoundryDatabase {
  if (!dbInstance) {
    dbInstance = new FoundryDatabase();
  }
  return dbInstance;
}
