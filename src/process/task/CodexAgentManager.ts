...
class CodexAgentManager extends BaseAgentManager<CodexAgentManagerData> implements ICodexMessageEmitter {
  ...
  private initAgent(data: CodexAgentManagerData) {
    // Initialize managers - following ACP architecture, pass message emitter
    ...
    this.bootstrap = (async () => {
      // Set Codex Agent's app config using Electron API in main process
      try {
  ...
  private performPostConnectionSetup(): void {
    try {
      ...
      // Delay session creation to first user message to avoid empty prompt issue
      // Session will be created with first user message - no session event sent here
    } catch (error) {
      // Output more detailed diagnostics
      const diagnostics = this.getDiagnostics();

      // Provide specific error message and suggestions
      ...
      // Don't throw error, let application continue running
      return;
    }
  }
  ...
  async sendMessage(data: { content: string; files?: string[]; msg_id?: string }) {
    ...
    try {
      ...
      const contentToSend = data.content?.includes(FOUNDRY_FILES_MARKER) ? data.content.split(FOUNDRY_FILES_MARKER)[0].trimEnd() : data.content;
      ...
      // If first message, send via newSession to avoid double message issue
      ...
      // These errors have typically been handled via MCP connection event stream
  ...
  /**
   * Unified confirmation method - manage all confirmations via addConfirmation
   * Following GeminiAgentManager and AcpAgentManager implementations
   */
  ...
  cleanup() {
    // Cleanup all managers - following ACP cleanup pattern
