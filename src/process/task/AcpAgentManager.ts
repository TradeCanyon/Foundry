    ...
    this.bootstrap = (async () => {
      ...
      this.agent = new AcpAgent({
        ...
        onSignalEvent: async (v) => {
          // Only send signal to frontend, don't update message list
  ...
  async sendMessage(data: { content: string; files?: string[]; msg_id?: string }): Promise<{
    ...
    try {
      ...
      if (data.msg_id && data.content) {
        ...
        if (contentToSend.includes(FOUNDRY_FILES_MARKER)) {
          contentToSend = contentToSend.split(FOUNDRY_FILES_MARKER)[0].trimEnd();
        }
        ...
        // Mark after first message is sent, regardless of whether there's presetContext
        if (this.isFirstMessage) {
