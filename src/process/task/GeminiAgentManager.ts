...
> {
  ...
  constructor(
    ...
    this.bootstrap = Promise.all([ProcessConfig.get('gemini.config'), this.getImageGenerationModel(), this.getMcpServers(), this.getGeminiApiKeyForTools(), this.getImageGenNativeModel()])
      .then(async ([config, imageGenerationModel, mcpServers, geminiApiKey, imageGenNativeModel]) => {
  ...
  private async getGeminiApiKeyForTools(): Promise<string | undefined> {
  ...
  private async getImageGenNativeModel(): Promise<string | undefined> {
    try {
      return await ProcessConfig.get('tools.imageGenNativeModel');
    } catch {
      return undefined;
    }
  }
  ...
  async sendMessage(data: { input: string; msg_id: string; files?: string[] }) {
    ...
    const result = await this.bootstrap
      .catch((e) => {
        ...
        // Need to sync before returning result
        // Why is this needed?
        // In some cases, messages need to be synced to local files. Due to async nature,
        // frontend may receive response but not get the latest message, so we need to wait for sync
