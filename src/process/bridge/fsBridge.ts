...
import { FOUNDRY_TIMESTAMP_SEPARATOR } from '@/common/constants';
...
export function initFsBridge(): void {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const request = client.get(
          ...
          {
            headers: {
              'User-Agent': 'Foundry-Preview',
              Referer: 'https://github.com/Foundry/Foundry',
            },
          },
          (response) => {
  ...
  ipcBridge.fs.createTempFile.provider(async ({ fileName }) => {
    try {
      ...
      if (fileExists) {
        ...
        const tempFileName = `${name}${FOUNDRY_TIMESTAMP_SEPARATOR}${timestamp}${ext}`;
  ...
  ipcBridge.fs.getFileMetadata.provider(async ({ path: filePath }) => {
    try {
      ...
      return {
        ...
        path: filePath,
        size: stats.size,
        type: '', // MIME type can be inferred from extension
      ...
      for (const filePath of filePaths) {
        try {
          ...
          if (exists) {
            ...
            const newFileName = `${name}${FOUNDRY_TIMESTAMP_SEPARATOR}${timestamp}${ext}`;
  ...
  ipcBridge.fs.scanForSkills.provider(async ({ folderPath }) => {
    ...
    try {
      ...
      // If no skills found in subdirectories, check if the selected folder itself is a skill
      if (skills.length === 0) {
