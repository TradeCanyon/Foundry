...
/**
 * Adapt Electron API for browser usage, establish communication bridge between renderer and main process, corresponds to injection in preload.ts
 */
if (win.electronAPI) {
  ...
  // Web environment - use WebSocket communication, automatically reconnect with session cookie after login
  ...
  // 1. Flush queued messages to ensure no events are lost after reconnection
  const flushQueue = () => {
  ...
  // 2. Simple exponential backoff reconnection, wait for server to accept new connections after login
