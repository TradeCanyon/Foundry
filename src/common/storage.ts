...
export interface IConfigStorageRefer {
  ...
  // Claude routing preference: 'auto' | 'cli' | 'api'
  // 'auto' = prefer CLI when available (default), 'cli' = always CLI, 'api' = always API
  'claude.routingPreference'?: 'auto' | 'cli' | 'api';
}
 ...
 */
export type ConversationSource = 'foundry' | 'telegram';
...
interface IChatConversation<T, Extra> {
  ...
  /** Conversation source, defaults to foundry */
...
export type TChatConversation =
  | IChatConversation<
      ...
      {
        workspace: string;
        customWorkspace?: boolean; // true: user-specified directory, false: system default directory
        webSearchEngine?: 'google' | 'default'; // Search engine config
        lastTokenUsage?: TokenUsageData; // Last token usage stats
  ...
  | IChatConversation<
      'image',
      {
        ...
        imageModel?: string;
        /** Provider config for image model */
        provider?: TProviderWithModel;
        /** Use Gemini native image generation (no provider needed) */
        useNative?: boolean;
        /** Optional workspace (for consistency with other conversation types) */
        workspace?: string;
        customWorkspace?: boolean;
...
export type ModelCapability = {
  type: ModelType;
  /**
   * Whether user manually selected this type. If true, user enabled it; if false, user disabled it; if undefined, use default
   */
...
export interface IProvider {
  ...
  model: string[];
  /**
   * Model capability labels. Tagged means supported, untagged means not supported
   */
  capabilities?: ModelCapability[];
  /**
   * Context token limit, optional field, only fill when known
   */
  contextLimit?: number;
}
