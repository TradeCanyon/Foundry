...
export const joinPath = (basePath: string, relativePath: string): string => {
  ...
  for (const part of parts) {
    if (part === '.' || part === '') {
      ...
      if (resultParts.length > 0) {
        resultParts.pop(); // Remove last part
      }
    } else {
  ...
  return result.replace(/\/+/g, '/'); // Replace multiple consecutive slashes with a single one
};

/**
 * @description Conversation-related message type declarations and processing
 */
...
// Unified type for all user interaction requests
...
/**
 * @description Transform backend message to frontend message format
 */
  ...
  const processedMessage = {
    ...
    content: {
      ...
      content: message.content.content.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt, imagePath) => {
        // If absolute path, http link, or data URL, keep unchanged
        ...
        // If relative path, join with workspace
