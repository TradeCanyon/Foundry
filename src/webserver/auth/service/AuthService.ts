...
export class AuthService {
  ...
  public static blacklistToken(token: string): void {
    // Use hash of token as key to avoid storing the raw token
   ...
   *
   * Reuses Web login token (audience: foundry-webui)
   *
  ...
  public static verifyWebSocketToken(token: string): TokenPayload | null {
    try {
      ...
      const decoded = jwt.verify(token, this.getJwtSecret(), {
        issuer: 'foundry',
        audience: 'foundry-webui', // Uses the same audience as Web login
      }) as RawTokenPayload;
