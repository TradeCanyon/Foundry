...
const DEV_PORT_ENV_KEYS = ['FOUNDRY_DEV_PORT', 'DEV_SERVER_PORT', 'PORT'] as const;
const LOGGER_PORT_ENV_KEYS = ['FOUNDRY_LOGGER_PORT', 'DEV_LOGGER_PORT', 'LOGGER_PORT'] as const;
...
const apkName = 'Foundry_' + packageJson.version + '_' + (process.env.arch || process.arch);
...
module.exports = {
  packagerConfig: {
    ...
    executableName: 'Foundry',
    out: path.resolve(__dirname, 'out'),
    tmpdir: path.resolve(__dirname, '../Foundry-tmp'),
    extraResource: [path.resolve(__dirname, 'public')],
    win32metadata: {
      CompanyName: 'foundry',
      FileDescription: 'AI Agent Desktop Interface',
      OriginalFilename: 'Foundry.exe', // 简化文件名
      ProductName: 'Foundry',
  ...
  makers: [
    ...
    ...(MakerSquirrel
      ? [
          new MakerSquirrel(
            {
              name: 'Foundry', // 必须与 package.json 的 name 一致
              authors: 'foundry', // 任意名称
